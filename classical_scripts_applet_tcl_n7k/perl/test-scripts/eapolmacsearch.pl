...
foreach my $hostname (@hosts){
        ...
        foreach my $key (sort{$a <=> $b} keys %$hash_ref){
                ...
                if ($more_mac == 1){
                        ...
                        #BEGIN der Ausgabe
                        #print "Begin der 2ten Ausgabe\n";
                        print "$hostname: $interface> $mac_search -> $macaddress\n";
...
sub help()
{
        print "\neapolmacsearch.pl is a little tool to check mac address on the EAP Authentication Databse on switches";
        print "\nSyntax:\t eapolmacsearch.pl -b ndl -m <mac>\n";
        exit 1;
}
