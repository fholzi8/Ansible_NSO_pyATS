...
my $atime = time;
my @hosts = ("list_of_hostnames");
my $username = "SNMPUser";
my $authpriv = "SNMP_Passwd!";
my $privpass = "SNMP_Secret";
my $script = "collisions";
my $oid = "1.3.6.1.4.1.9.2.2.1.1.25";
...
my $new_file = "/var/www/html/switch-error/txt/".$script.".conf";
my $old_file = "/var/www/html/switch-error/txt/".$script."_old.conf";
open(FILE,">$new_file");
...
my $current_file = "/scripts/html/switch-error/current_".$script."_error.html";
open(FILE,">$current_file");
print FILE $combuf;
close FILE;
my $file = "/scripts/html/switch-error/check_".$script."_error.html";
open(FILE,">$file");
print FILE $msgbuf;
close FILE;
