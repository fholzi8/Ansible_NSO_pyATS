...
#           Modified for Cisco IOS by Ivan Pepelnjak (ipepelnjak@nil.com)
#
...
#
# This implementation uses TCP only for DNS queries. UDP sockets cannot be 
...
#
# Modifications for Cisco IOS:
# * Removed the URI support to minimize the package dependencies
# * Removed the UDP support
# * Removed the fileevent call and replaced it with a wait loop in dns::wait
# * Fixed the zero-characters-read case that could arise in TcpEvent due to 
#   wait loop
# * Added the IOS-specific ::dns::nameservers procedure
#
# -------------------------------------------------------------------------
...
switch -exact $::tcl_platform(platform) {
    ...
    ios {
        proc ::dns::nameservers {} {
	    set nameservers {}
            set nslist [exec "show host | include Name servers"]
            if { $nslist == "" } {
		return -code error "IP domain lookup disabled on the router"
	    }
            return [regexp -inline -all {[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+} $nslist]
	}
    }
