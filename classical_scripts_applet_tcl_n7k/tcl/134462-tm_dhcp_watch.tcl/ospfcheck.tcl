#	revision 1.3 2008-06-30 by user
#	initial revision
#
#	description:	This script displays OSPF neighbors, including OSPF area, sorted by process ID
#	ios config:
#			* download the file into flash:.tcl/ospfcheck.tcl
#			* download the file into flash:.tcl/host.tcl
#		  * configure alias exec ospfcheck tclsh flash:ospfcheck.tcl for example
#
#			**invoke with ospfcheck
#
...
namespace eval DnsHost {
	...
	if { $ts == "A" } {
	    ...
	    global host_event
            ...
            set host_event ""
	    # TIMEOUT is hardcoded should be configurable
...
proc printProcess {pid} {  
	...
	foreach line [split $cmdtext "\n"] {
		if {[regexp -nocase {neighbor ([0-9.]+).*interface address ([0-9.]+)} $line ignore id ifaddr]} {     
			...
			set neighbor(NAME) [exec "tclsh flash:.tcl/ospf_resolve_dns.tcl $id"]
