# =================================================================================
# CHANGE LOG
# 30 sept 08 - rv - added PN in addition to P0 support
# 30 sept 08 - rv - added reason codes to udp
# =================================================================================
   ...
   if { $scantype == "P" } {
      if { $hostexist ==1 } {
          ...
          if {$hostexist == 1 } {
             ...
             puts -nonewline stdout "PORT     STATE      SERVICE"
             ...
             puts -nonewline stdout "$port/$proto   $state     $service"
             if {$reasoncode == 1} { puts -nonewline stdout "    $reason"} 
             puts stdout ""
          } 
...
proc getservices { loadpath } {
     global svctcp
     global svcudp
     global svcarraymax

     for {set i 1} {$i < $svcarraymax} {incr i} {
          set svctcp($i) "."
          set svcudp($i) "."
     }
     set svcfile "services.list"
     if [catch {open $loadpath$svcfile r} fileId] {
          puts stderr "Cannot open services file"
     } else {
          set services [read $fileId]
          close $fileId
     }

     foreach record $services {
          ...
          switch $localproto {
              tcp {set svctcp($localport) $localsvc }
...
foreach arg $argv  {
    ...
    if {$scantype != "P"} {
        set loadpath [ setloadpath $fullpath]
